...
records the full list of sheets when it generates the file and reads it back
automatically. That includes sheets you brought in with `--include-node`.

Workbooks generated by g3mt 2.2.0 and earlier recorded a single path rather than
a node list; those keep validating exactly as they always did. (If you
